...
namespace CSharpNeat
{
    class Neat
    {
        ...
        public void initializePop(int pop, int numInputNodes, int numOutputNodes)
        {
            for (int i = 0; i < pop; i++)
            {
                population.Add(new Indiv(numInputNodes, numOutputNodes, rand));
            }
        }
        ...
        public void mutatePop(List<Indiv> pop)
        {
            for (int i = 0; i < pop.Count; i++) 
            {
                if (rand.NextDouble() > .80)
                {
                    innovationCount = pop[i].mutate(innovationCount);
                }
            }
        }
        ...
        public Indiv crossOver(Indiv parent1, Indiv parent2)
        {
            ...
            int networkSize = 0;
            Indiv child = new Indiv(parent1.NumInputNodes, parent2.NumOutputNodes, rand);
